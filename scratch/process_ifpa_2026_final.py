...
# Full known course names for IFPA 2026 to fix fragmentation
COURSE_NAMES = [
...
def clean_text(text):
    # Remove excessive spaces between characters but keep words separate
    # Example: "N o t u r n o" -> "Noturno"
    # This is tricky, let's just look for keywords instead
    return text

def detect_metadata(full_text):
    # Search for course
    found_course = None
    for c in COURSE_NAMES:
        # Check if course name is present even if characters are slightly split
        pattern = "".join([re.escape(char) + r"\s*" for char in c])
    ...
    for camp in CAMPUSES:
        pattern = "".join([re.escape(char) + r"\s*" for char in camp])
        if re.search(pattern, full_text, re.IGNORECASE):
            found_campus = camp
            break
    ...
    if re.search(r"L\s*I\s*C\s*E\s*N\s*C\s*I\s*A\s*T\s*U\s*R\s*A", full_text, re.IGNORECASE):
        found_degree = "LICENCIATURA"
    elif re.search(r"B\s*A\s*C\s*H\s*A\s*R\s*E\s*L\s*A\s*D\s*O", full_text, re.IGNORECASE):
        found_degree = "BACHARELADO"
    elif re.search(r"T\s*E\s*C\s*N\s*O\s*L\s*O\s*G\s*I\s*A", full_text, re.IGNORECASE):
        found_degree = "TECNOLOGIA"
        
    # Search for shift
    found_shift = None
    shifts = ["VESPERTINO", "NOTURNO", "MATUTINO", "INTEGRAL"]
    for s in shifts:
        pattern = "".join([re.escape(char) + r"\s*" for char in s])
        if re.search(pattern, full_text, re.IGNORECASE):
            found_shift = s
            break
...
def process_ifpa_2026():
    ...
    with pdfplumber.open(PDF_PATH) as pdf:
        # Keep track of current state across pages
        state = {
            "course": None,
            "campus": None,
            "degree": "GRADUAÇÃO",
        ...
        for i, page in enumerate(pdf.pages):
            ...
            # Check for new header on this page
            # We look at the first few lines of the page
            header_text = "\n".join(lines[:10])
            c, camp, d, s = detect_metadata(header_text)
            if c: state["course"] = c
            ...
            for line in lines:
                # Also check each line for potential mid-page header changes
                if "Graduação" in line or "Campus" in line:
                    c_l, camp_l, d_l, s_l = detect_metadata(line)
                    if c_l: state["course"] = c_l
                    if camp_l: state["campus"] = camp_l
                    if d_l != "GRADUAÇÃO": state["degree"] = d_l
                    if s_l: state["shift"] = s_l
                ...
                if "Aprovado(a)" in line or "Classificado(a)" in line:
                    ...
                    if status_idx != -1 and status_idx >= 3:
                        ...
                        try:
                            ...
                            if norm_quota and state["course"] and state["campus"]:
            ...
            if i % 10 == 0:
                print(f"Processed {i+1}/{len(pdf.pages)} pages. Current: {state['course']} ({state['campus']})")

    if not os.path.exists(OUTPUT_DIR):
        os.makedirs(OUTPUT_DIR)
        
    for (campus, course, degree, shift), candidates in offerings_data.items():
        if not candidates or shift == "N/A": continue
